...
namespace DieMob
{
    ...
    public class DieMobMain : TerrariaPlugin
    {
                ...
                try
                {
                    ...
                    for (int i = 0; i < Main.npc.Length; i++)
                    {
                        if (Main.npc[i].active)
                        {
                            ...
                            Region tsregion = DieMobMain.getValidTopRegionFromCoordinates((int)(npc.position.X / 16f), (int)(npc.position.Y / 16f));
                            if (tsregion != null)
                            {
                                DieMobRegion region = DieMobMain.GetRegionByName(tsregion.Name);
                                if (region == null)
                                    return;
        ...
        private static Region getValidTopRegionFromCoordinates(int pX, int pY)
        {
            IEnumerable<Region> regions = TShock.Regions.InAreaRegion(pX, pY);
            Region result;
            ...
            {
                Region region = TShock.Regions.GetTopRegion(regions);
                if (region == null || region.Z > 50)
                {
                    result = null;
                }
                else
